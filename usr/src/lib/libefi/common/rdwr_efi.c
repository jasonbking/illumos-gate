    ...
    diskaddr_t *last_lbap)
{
	 ...
	 * (efi_nparts - 2) partition since that should be the reserved
	 * partition (which is checked later).
	 */
	for (i = 0; i < l->efi_nparts - 2; i++) {
	...
	if (l->efi_parts[l->efi_nparts - 1].p_tag == V_RESERVED) {
		if (resv_p != NULL) {
			if (efi_debug) {
				(void) fprintf(stderr, "%s: duplicate reserved "
				    "partitions present\n", __func__);
			}
			return (-1);
		}
		...
		/*
		 * The reserved partition is not the last partition. We
		 * wrote an error message for this earlier, so no need to
		 * duplicate it here.
		 */
		return (-1);
	} else {
