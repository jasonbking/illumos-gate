...
#include <sys/pci.h>
#include <sys/pcie.h>
/* arguably a bug, but must these must appear before sys/pcie_impl.h */
#include <sys/ddifm.h>
...
{
	extern caddr_t alloc_vaddr(size_t, paddr_t);

	const uint64_t ecam_base = mcfg_mem_base;
	 ...
	 */
	pcie_cfgspace_vaddr = (uintptr_t)alloc_vaddr(PCIE_CFGSPACE_SIZE,
	    PCIE_CFGSPACE_ALIGN);
