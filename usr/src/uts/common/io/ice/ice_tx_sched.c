...
ice_tx_sched_add_vsi_node(ice_t *ice, ice_vsi_t *vsi)
{
	...
	uint_t			total[ICE_SCHED_NODE_MAX_DEPTH] = { 0 };
	add_vsi_args_t		args = { 0 };
	uint_t			i, per_node, qgrp_layer, parents;
	bool			ret = false;
	...
	if (!ice_tx_sched_add_children(ice, node, buf, add_vsi_nintermediate,
	    &args)) {
		/*
		 * Do our best to clean up any intermediate nodes we
		 * created before failing. node is either an existing
		 * ancestor or root, so we only remote node's children
		 * but not root.
		 */
		while (node->isn_nchildren > 0) {
			if (!ice_tx_sched_del_subtree(ice,
			    node->isn_children[0])) {
				ice_error(ice, "failed to clean up "
				    "partially created TX scheduler nodes; "
				    "leaking scheduler resources");
				break;
			}
		}
		goto done;
	...
	/*
	 * Calculate how many nodes per level under the VSI node we need to
	 * add. The topology can have more than one layer between the VSI
	 * and the queue-group layer (the layer directly parenting TX queue
	 * leaf nodes), and ice_tx_sched_add_children()'s recursion applies
	 * whatever count we give it here identically to every parent at
	 * that layer -- so a value can't simply be "the total number of
	 * nodes this layer needs" once there's more than one node at the
	 * layer above (that total could exceed a single parent's allowed
	 * child count). So we do this in two passes:
	 *
	 * 1. Working from the queue-group layer up to (but not including)
	 *    the VSI layer, compute the total number of nodes needed at
	 *    each layer, assuming every node is packed to its maximum
	 *    allowed number of children.
	 *
	 * 2. Working back down from the VSI layer, convert each layer's
	 *    total into the number of children each individual parent at
	 *    that layer should get, by dividing by the total node count of
	 *    the layer immediately above (which is how many parents exist
	 *    for this layer). Since parent_total = ceil(total / max_sibs),
	 *    ceil(total / parent_total) is always <= max_sibs, so this can
	 *    never ask a node to exceed its allowed child count.
	 *
	 * Which is a lot of words to say that in the end we want to
	 * create enough intermediate nodes so that the leaf nodes
	 * (each one represents a TX queue) are evenly distributed amongst
	 * their parents (so things look nice and balanced).
	 *
	 * If we add support for dynamically adding/removing TX rings, or
	 * offloding TX flow management to NICs, this will likely need to
	 * become more sophisticated. We might also need to look at
	 * how supporting multiple VSIs might impact the topology.
	 */
	qgrp_layer = ice->ice_tx_max_layers - 2;

	per_node = vsi->ivsi_ntxq;
	ASSERT3U(per_node, !=, 0);

	for (i = qgrp_layer; i > args.ava_vsi_layer; i--) {
		uint16_t max = ice->ice_tx_sched_max_sibs[i + 1];

		per_node = P2ROUNDUP(per_node, max) / max;
		ASSERT3U(per_node, !=, 0);
	...
	parents = 1;
	for (i = args.ava_vsi_layer + 1; i <= qgrp_layer; i++) {
		uint16_t want = P2ROUNDUP(total[i], parents) / parents;

		ASSERT3U(want, <=, ice->ice_tx_sched_max_sibs[i]);
	...
	if (!ret) {
		/*
		 * The VSI node (and everything under it) belongs
		 * exclusively to this VSI, so on failure we try avoid
		 * leaking scheduler resources by attempting to back out
		 * the whole subtree.
		 */
		if (!ice_tx_sched_del_subtree(ice, node)) {
			ice_error(ice, "failed to clean up partially "
			    "created VSI TX scheduler nodes; leaking "
			    "scheduler resources");
		}
	}
...
/*
 * Recursively delete an entire subtree rooted at node. Children must
 * be deleted before the parent. Used to cleanup a partially created
 * tree after failure as well as when a VSI is deleted.
 *
 * Unlike ice_tx_sched_free_nodes(), which only frees our SW bookkeeping and
 * assumes the HW side is already gone (e.g. detach or a full reset), this
 * actually issues the HW delete commands (except for leaf nodes, which per
 * the datasheet are implicitly deleted along with their associated TX ring
 * or RDMA QSet, and so are only removed from our SW bookkeeping here).
 *
 * On success, node has been deleted from HW (as appropriate) and freed, and
 * must not be referenced by the caller again. On failure, there's little
 * more we can do, so we stop as soon as a delete fails. Any remaining
 * descandants are left as is (and likely leaked).
 */
bool
ice_tx_sched_del_subtree(ice_t *ice, ice_sched_node_t *node)
{
	ASSERT(MUTEX_HELD(&ice->ice_tx_sched_lock));

	/*
	 * Deleting a child removes it from node->isn_children and shifts
	 * the remaining entries down, so we can always operate on index 0
	 * until there are none left.
	 */
	while (node->isn_nchildren > 0) {
		if (!ice_tx_sched_del_subtree(ice, node->isn_children[0])) {
			return (false);
		}
	}

	return (ice_tx_sched_del_elt(ice, node,
	    node->isn_type != ICE_TX_SCHED_ET_LEAF));
}
