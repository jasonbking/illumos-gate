...
ice_stat_pf_update(ice_t *ice, bool init)
{
	...
	/*
	 * Unlike the above, this isn't a delta counter maintained by
	 * hardware; it's an instantaneous reading retrieved via an admin
	 * queue command. Its failure isn't considered service effecting
	 * either; we simply leave the previously read values in place.
	 */
	if (ice_cmd_get_sensor_reading(ice, &temp, &warn, &crit, &fatal)) {
		ks->ipk_temp.value.i32 = temp;
		ks->ipk_temp_warning_threshold.value.ui32 = warn;
		ks->ipk_temp_critical_threshold.value.ui32 = crit;
		ks->ipk_temp_fatal_threshold.value.ui32 = fatal;
	}
}
...
ice_stats_init(ice_t *ice)
{
	...
	kstat_named_init(&ks->ipk_temp, "temp", KSTAT_DATA_INT32);
	kstat_named_init(&ks->ipk_temp_warning_threshold,
	    "temp_warning_threshold", KSTAT_DATA_UINT32);
	kstat_named_init(&ks->ipk_temp_critical_threshold,
	    "temp_critical_threshold", KSTAT_DATA_UINT32);
	kstat_named_init(&ks->ipk_temp_fatal_threshold,
	    "temp_fatal_threshold", KSTAT_DATA_UINT32);
