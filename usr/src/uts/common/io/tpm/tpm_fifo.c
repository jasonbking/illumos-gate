...
tpm_tis_get_burstcount(tpm_t *tpm, uint16_t *burstp)
{
	...
	ret = tpm_wait(tpm, tis_burst_nonzero, tpm->tpm_timeout_d, false,
	    __func__);
	if (ret != 0) {
		return (ret);
	}
...
tis_fifo_make_ready(tpm_t *tpm, clock_t to)
{
	...
	ret = tpm_wait(tpm, tis_is_ready, to, false, __func__);
	mutex_exit(&tpm->tpm_lock);
...
tis_expecting_data(tpm_t *tpm, bool *expp)
{
	...
	ret = tpm_wait(tpm, tis_status_valid, tpm->tpm_timeout_c,
	    tis->ttis_has_sts_valid_int, __func__);
	if (ret != 0) {
		return (ret);
	}
...
tis_send_data(tpm_t *tpm, uint8_t *buf, uint32_t amt)
{
	 ...
	 * bytes to write at one time. Once we write burstcount bytes, we
	 * must wait for the TPM to report a burstcount > 0 before writing
	 * more bytes.
	 */
	do {
		/*
...
retry:
	...
	ret = tpm_wait(tpm, tis_status_valid, tpm->tpm_timeout_c, false,
	    __func__);
...
tis_request_locality(tpm_t *tpm, uint8_t locality)
{
	...
	ret = tpm_wait(tpm, tis_is_locality_active, tpm->tpm_timeout_a, true,
	    __func__);
	mutex_exit(&tpm->tpm_lock);
