...
tpm_exec_internal(tpm_client_t *c)
{
	 ...
	 * hold it while assembling and executing the command and release
	 * after completion. As such, we just unilaterally set the state to
	 * TPM_CLIENT_CMD_RECEPTION to satisify tpm_dispatch_cmd()s
	 * expectations even though we don't use it with kernel consumers.
	 */
...
tpm_exec_cmd(tpm_t *tpm, tpm_client_t *c, tpm_cmd_t *cmd)
{
	...
	DTRACE_PROBE4(cmd__exec, uint32_t, cc, tpm_client_t *, c,
	    tpm_cmd_t *, cmd, uint32_t, tpm_cmdlen(cmd));
	...
	DTRACE_PROBE4(cmd__done, uint32_t, ret, uint32_t, tpm_cmd_rc(cmd),
	    tpm_cmd_t *, cmd, uint32_t, tpm_cmdlen(cmd));
    ...
    const char *func)
{
	...
	exp_done = (tpm->tpm_wait != TPM_WAIT_POLLONCE) ? deadline : now + dur;
