...
crb_go_idle(tpm_t *tpm)
{
	...
	ret = tpm_wait(tpm, crb_is_go_idle_done, tpm->tpm_timeout_c, false,
	    __func__);
	if (ret != 0) {
		mutex_exit(&tpm->tpm_lock);
		return (ret);
...
crb_go_ready(tpm_t *tpm)
{
	...
	ret = tpm_wait(tpm, crb_is_go_ready_done, tpm->tpm_timeout_c, true,
	    __func__);
	if (ret == 0) {
...
crb_request_locality(tpm_t *tpm, uint8_t locality)
{
	...
	ret = tpm_wait(tpm, crb_request_locality_done, tpm->tpm_timeout_c, true,
	    __func__);
	if (ret != 0) {
